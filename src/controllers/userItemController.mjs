...
const getUserItemDetail = async (req, res, next) => {
  try {
    ...
    if (!userItem) {
      return res.status(errors.HTTP.CODE.NOT_FOUND).send(
        responseError(errors.HTTP.CODE.NOT_FOUND, errors.HTTP.STATUS.NOT_FOUND, 'User item not found')
      );
    }
    res.status(success.HTTP.CODE.OK).send(
...
const updateUserItem = async (req, res, next) => {
  try {
    ...
    if (!existingUserItem) {
      return res.status(failure.HTTP.CODE.NOT_FOUND).send(
        responseError(failure.HTTP.CODE.NOT_FOUND, failure.HTTP.STATUS.NOT_FOUND, 'User item not found')
      );
    }
