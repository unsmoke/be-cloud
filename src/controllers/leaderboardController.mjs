...
const getProvinceLeaderboard = async (req, res, next) => {
  try {
    ...
    const provinceLeaderboards = await leaderboardService.fetchLeaderboardProvince(province_id, sort_by, page, per_page);
...
const getCityLeaderboard = async (req, res, next) => {
  try {
    ...
    const cityLeaderboards = await leaderboardService.fetchLeaderboardCity(city_id, sort_by, page, per_page);
    res.status(success.HTTP.CODE.OK).send(
