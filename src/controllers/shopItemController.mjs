  ...
  try {
    ...
    if (!shopItem) {
      return res.status(failure.HTTP.CODE.NOT_FOUND).send(
        responseError(failure.HTTP.CODE.NOT_FOUND, failure.HTTP.STATUS.NOT_FOUND, 'Shop item not found')
      );
    }
  ...
  try {
    ...
    if (!existingShopItem) {
      return res.status(errors.HTTP.CODE.NOT_FOUND).send(
        responseError(errors.HTTP.CODE.NOT_FOUND, errors.HTTP.STATUS.NOT_FOUND, 'Shop item not found')
      );
    }
